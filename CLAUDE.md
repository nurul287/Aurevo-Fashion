...
**Bulk product import** (`src/pages/admin/admin-imports-page.tsx`, route `/admin/imports`) — admin drag-and-drop upload (`.xlsx`/`.csv`/`.json`/`.ndjson`) into Aurevo.BE's async import pipeline. `src/services/imports/` hooks: `useUploadImport` (`apiFetchForm`), `useImportJob(jobId)` polling every 2s via `refetchInterval` until the job hits a terminal status (the first `refetchInterval` usage in this codebase), `useImportRows`, `useRetryImportJob`. Progress bar follows the same manual-`<div>` idiom as `admin-ai-metrics-page.tsx`'s tool-usage bars (no shadcn `Progress` component in this project). Template download reuses `apiDownloadFile`. Full backend architecture — including the standalone scraper package that can also feed this endpoint — in Aurevo.BE's [`docs/10-bulk-import-pipeline.md`](../Aurevo.BE/docs/10-bulk-import-pipeline.md).
