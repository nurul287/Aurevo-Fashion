...
Logged-in product-level favorites against BE `/api/wishlist`. Product-card heart toggles add/remove; `/dashboard/wishlist` lists saved products with add-to-cart (variants included in the wishlist payload). Guests are prompted to sign in — no guest wishlist.
